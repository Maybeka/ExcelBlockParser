...
test('opens the project Python editor with syntax highlighting', async ({ page }) => {
  ...
  await expect(dialog.getByRole('button', { name: 'Prepare input' })).toBeVisible()
...
test('previews and saves generated text files through the host bridge', async ({ page }) => {
  ...
  await expect(dialog.getByRole('tab', { name: 'Files (3)' })).toHaveAttribute('aria-selected', 'true')
  ...
  await dialog.getByRole('tab', { name: 'Files (3)' }).click()
