...
test.describe('M3 workspace layout', () => {
  ...
  test('opens About from the project actions menu', async ({ page }) => {
    ...
    await expect(dialog.locator('.about-dialog-brand span')).toHaveText(/^v\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/)
    await expect(dialog.getByText('Current development v3')).toBeVisible()
  })

  test('exposes the default-on staged loading switch in project settings', async ({ page }) => {
    await page.goto('/')
    ...
    await stagedLoading.getByRole('checkbox').uncheck()
