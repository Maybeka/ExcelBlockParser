...
import { diagnosticFocusTarget, orderDiagnostics } from '../services/diagnostics'
...
describe('diagnostics coordinator', () => {
  it('resolves a cross-workbook focus target before UI navigation', () => {
    expect(diagnosticFocusTarget(project, { code: 'sheet-missing', severity: 'error', blockId: 'block', message: 'missing' })).toMatchObject({ workbookId: 'b', sheetName: 'T', blockId: 'block' })
  })
