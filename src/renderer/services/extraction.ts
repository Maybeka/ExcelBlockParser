...
import { columnKeyFromHeaders } from './columnKey'
...
export function suggestColumnMappings(values: unknown[][], range: CellRange, headerRows: number[]): ColumnMapping[] { const headers = new Set(headerRows); return generateColumnMappings(range).map((column, offset) => { const headerParts = headerRows.map(row => values[row]?.[offset]).filter(value => value != null && String(value).trim()).map(value => String(value).trim()); const key = headerParts.length ? columnKeyFromHeaders(headerParts) : column.key; return { ...column, suggestedKey: key, key, type: inferColumnType(values.filter((_, index) => !headers.has(index)).map(row => row[offset])) } }) }
