  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
        <Tooltip title="Stored for downstream code generation; not executed during parsing.">
          <span style={{ fontSize: 11, color: '#999', cursor: 'pointer', userSelect: 'none' }} onClick={onToggle}>
            {expanded ? <CaretDownOutlined /> : <CaretRightOutlined />}
            {' '}Downstream Properties {properties.length ? `(${properties.length})` : ''}
          </span>
        </Tooltip>
        {expanded && (
        ...
        return (
          <div key={property.id} style={{ marginBottom: 6 }}>
            <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
              <Input
                size="small" value={property.label} placeholder="name"
            ...
            {property.expression && validation.valid && <div style={{ fontSize: 10, color: '#52c41a' }}>✓ Valid</div>}
            {duplicateKeys.has(property.label?.trim() || '') && <div style={{ fontSize: 10, color: '#ff4d4f' }}>Duplicate property label</div>}
          </div>
        )
      })}
      {expanded && !properties.length && <div style={{ fontSize: 11, color: '#bbb' }}>No computed properties</div>}
    </div>
  )
}
